...
double Avgsumb1_A, Avgsumb2_A, Avgsumb3_A, Avgsumb4_A, Avgsumb5_A, Avgsumb1_B, Avgsumb2_B, Avgsumb3_B, Avgsumb4_B, Avgsumb5_B;
...
void AggregateSamples(){
  double b1_A[10]={},b2_A[10]={},b3_A[10]={},b4_A[10]={},b5_A[10]={},b1_B[10]={},b2_B[10]={},b3_B[10]={},b4_B[10]={},b5_B[10]={};
  double sumb1_A=0, sumb2_A=0, sumb3_A=0, sumb4_A=0, sumb5_A=0;
  double sumb1_B=0, sumb2_B=0, sumb3_B=0, sumb4_B=0, sumb5_B=0;
  int c;
  for (c=0; c<10; c++) {
        b1_A[c] = throughputWifi;
        b2_A[c] = totalBytesReceivedSumWifi;
        b3_A[c] = vet_g_signalDbmAvgWifi;
        b4_A[c] = vet_g_noiseDbmAvgWifi;
        b5_A[c] = vet_g_SNRWifi;

        b1_B[c] = throughputWave;
        b2_B[c] = totalBytesReceivedSumWave;
        b3_B[c] = vet_g_signalDbmAvgWave;
        b4_B[c] = vet_g_noiseDbmAvgWave;
        b5_B[c] = vet_g_SNRWave;
        // acumuladores
        sumb1_A = sumb1_A+b1_A[c];
        sumb2_A = sumb2_A+b2_A[c];
        sumb3_A = sumb3_A+b3_A[c];
        sumb4_A = sumb4_A+b4_A[c];
        sumb5_A = sumb5_A+b5_A[c];

        sumb1_B = sumb1_B+b1_B[c];
        sumb2_B = sumb2_B+b2_B[c];
        sumb3_B = sumb3_B+b3_B[c];
        sumb4_B = sumb4_B+b4_B[c];
        sumb5_B = sumb5_B+b5_B[c];
...
if (c>=10){
    Avgsumb1_A=sumb1_A/10;
    Avgsumb2_A=sumb2_A/10;
    Avgsumb3_A=sumb3_A/10;
    Avgsumb4_A=sumb4_A/10;
    Avgsumb5_A=sumb5_A/10;

    Avgsumb1_B=sumb1_B/10;
    Avgsumb2_B=sumb2_B/10;
    Avgsumb3_B=sumb3_B/10;
    Avgsumb4_B=sumb4_B/10;
    Avgsumb5_B=sumb5_B/10;


    memset(b1_A, 0, sizeof(b1_A));
    memset(b2_A, 0, sizeof(b2_A));
    memset(b3_A, 0, sizeof(b3_A));
    memset(b4_A, 0, sizeof(b4_A));
    memset(b5_A, 0, sizeof(b5_A));

    memset(b1_B, 0, sizeof(b1_B));
    memset(b2_B, 0, sizeof(b2_B));
    memset(b3_B, 0, sizeof(b3_B));
    memset(b4_B, 0, sizeof(b4_B));
    memset(b5_B, 0, sizeof(b5_B));

}
  fflush(stdin);
  Simulator::Schedule (MilliSeconds (100), &AggregateSamples);
...
void InterfaceManager(NodeContainer ueNode, UdpClientHelper dlClient, UdpClientHelper wdlClient, uint16_t dport, uint16_t wdport) {
    ...
    //
        ...
        //
        // if (Avgsumb2_B > Avgsumb2_A){
        //           sumPointsIntB = sumPointsIntB + 1;
        //         }  else {
        //           sumPointsIntA = sumPointsIntA + 1;
        // }
        //
        // if (Avgsumb3_B > Avgsumb3_A){
        //           sumPointsIntB = sumPointsIntB + 1;
        //         }  else {
        //           sumPointsIntA = sumPointsIntA + 1;
        // }
        //
        // if (Avgsumb4_B > Avgsumb4_A){
        //           sumPointsIntA = sumPointsIntA + 1;
        //         }  else {
        //           sumPointsIntB = sumPointsIntB + 1;
        // }
        //
        // if (Avgsumb5_B > Avgsumb5_A){
        //           sumPointsIntB = sumPointsIntB + 1;
        //         }  else {
        //           sumPointsIntA = sumPointsIntA + 1;
        // }
  ...
  void experiment(int &numberOfUEs, const std::string phyMode1, const std::string phyMode2, bool verbose, Ptr<WifiPhyStats> m_wifiPhyStats, int m_mobility, double m_txp, uint32_t m_protocol, int m_numberOfInterfaces, double duration)
  {
...
// conection with gazebo
           ...
           sinkApplications2.Start (Seconds (0));
          // for mobility without Gazebo
...
//connection Gazebo
          ...
          uint32_t portNumber = 10;
            ...
            //  {
                ...
                for (uint32_t i=0; i < ueNode.GetN ();i++){
                            sinkApplications.Add (packetSinkHelper.Install (ueNode.Get (i)));
                          }
              ...
              uint32_t portNumber2 = 20;
                ...
                //  {
                    ...
                    for (uint32_t i=0; i < ueNode.GetN ();i++){
                                sinkApplications2.Add (packetSinkHelper2.Install (ueNode.Get (i)));
                              }
        ...
        //  double interPacketInterval = 100;
         ...
         //  dlClient.SetAttribute("StartTime", TimeValue(MilliSeconds(0)));
         // // for mobility without Gazebo
    ...
    //    CalculatePhyRxDrop (m_wifiPhyStats);
     ...
     Simulator::Schedule(Seconds(0.1), &AggregateSamples);
...
// conection Gazebo
  ...
  //    Ptr <Socket> SetupPacketReceive (Ipv4Address addr, Ptr <Node> node );
    ...
    //  std::cout << "F1 Tx Packets;F1 Rx Packets;F1 Tx Bytes;F1 Rx Bytes;F2 Tx Packets;F2 Rx Packets;F2 Tx Bytes;F2 Rx Bytes;Collisions\n";
      ...
      {
          experiment(numberOfUEs, phyMode1, phyMode2, verbose, m_wifiPhyStats, m_mobility, m_txp, m_protocol, m_numberOfInterfaces, duration);

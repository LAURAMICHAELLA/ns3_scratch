...
// Variable definition (all generic variables starts with "m_")
  int m_xNodes = 2; // Mesh width in number of nodes
  int m_yNodes = 5; // Mesh heigth in number of nodes
  ...
  bool m_newFlowFile = true; // Clear flow .csv file
  bool m_drawAnim = true; // Enable netanim .xls output
  int m_numberUavMdc=5; // number of UAVs
  ...
  //std::string m_trName ("mesh-COVID"); ///< trace file name
...
// Command line options
  ...
  cmd.AddValue ("number of UAVs", "Set the number of UAVs MDC", m_numberUavMdc);
  cmd.Parse (argc, argv);
...
// Node container creation (all node containers starts with "nc_")
  ...
  NodeContainer nc_uav; //Contains uavs NODES

  nc_mesh.Create (m_xNodes * m_yNodes);
  nc_all.Create (2);
  nc_all.Add (nc_mesh);
  nc_uav. Create (m_numberUavMdc); // 5 uavs to get data from sink
  nc_all.Add (nc_uav);
...
// Set YansWifiChannel
  ...
  //NetDeviceContainer de_uav = wifi.Install (wifiPhy, wifiMac, nc_uav);
...
// Set startup positions for nodes
  ...
  mobilityUAVs.SetPositionAllocator ("ns3::GridPositionAllocator",
                                    "MinX", DoubleValue (m_distAP),
                                    "MinY", DoubleValue (0.0),
                                    "DeltaX", DoubleValue (m_distNodes),
                                    "DeltaY", DoubleValue (m_distNodes),
  ...
  // if (m_numberUavMdc > 0) // Set mobility for UAVs
  // { //if mission nodes
  //
  //   int t1_uav = m_distAP / 4;
  //   int t2_uav = m_distAP + ((m_xNodes - 1) * m_distNodes) + (m_distAP / 4);
  //   int t3_uav = m_distAP / -2;
  //   int t4_uav = (m_yNodes * m_distNodes) + (m_distAP / 4);
  //   sprintf (tmp_char_2, "%d|%d|%d|%d", t1_uav, t2_uav, t3_uav, t4_uav);
  //   mobilityUAVs.SetMobilityModel ("ns3::RandomWalk2dMobilityModel",
  //                                 "Mode", StringValue ("Time"),
  //                                 "Time", StringValue ("2s"),
  //                                 "Bounds", StringValue (tmp_char_2));
  // }
...
// Set Internet stack and config IPs for interfaces (all interfaces starts with "if_")
  ...
  // Ipv4AddressHelper addrUAV;
  // addrUAV.SetBase ("192.168.0.0", "255.255.255.0");
  // Ipv4InterfaceContainer if_uav = addrUAV.Assign (de_uav);
...
/////PING FOR TESTS
        ...
        appPingInternet2.Start (Seconds (0));
         ...
         // ApplicationContainer appPingInternet3 = ping3.Install (nc_all.Get(m_xNodes * m_yNodes + 1 + m_numberUavMdc));
...
// animation1.UpdateNodeColor(nc_mesh.Get(m_xNodes * m_yNodes), 0, 255, 0);
 ...
 // for (tmp_uav = 2 + m_xNodes * m_yNodes ; tmp_uav < m_xNodes * m_yNodes + 2 + m_numberUavMdc; tmp_uav++)
 // {
 //  sprintf (tmp_char_2, "%d-UAV", tmp_uav);
 // AnimationInterface animation ("m_animFile.xml");
 //   animation.UpdateNodeDescription (nc_all.Get (tmp_uav), tmp_char_2);
 //
 //  }

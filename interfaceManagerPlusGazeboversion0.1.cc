...
void InterfaceManager(NodeContainer ueNode, UdpClientHelper dlClient, UdpClientHelper wdlClient, uint16_t dport, uint16_t wdport) {
    ...
    int change=0,old_change=0, numberofchanges=0, numberofexecution=0;
...
void experiment(int &numberOfUEs, const std::string phyMode1, const std::string phyMode2, bool verbose, Ptr<WifiPhyStats> m_wifiPhyStats, int m_mobility, uint32_t m_protocol, int m_numberOfInterfaces, double duration)
{
...
int main (int argc, char *argv[])
{
  ...
  //conection Gazebo
    ...
    double duration = 1200.0; //seconds if experiment in sumo 90.0 gazebo 140
...
//    cmd.AddValue ("txp", "Transmit power (dB), e.g. txp=7.5", m_txp);
  ...
  //  void CalculateThroughput2 (Ptr<WifiPhyStats> m_wifiPhyStats);
    ...
    std::cout << "Duration of Experiment NS-3!--->" << duration << "s" << std::endl;
  //  std::cout << "F1 Tx Packets;F1 Rx Packets;F1 Tx Bytes;F1 Rx Bytes;F2 Tx Packets;F2 Rx Packets;F2 Tx Bytes;F2 Rx Bytes;Collisions\n";
    ...
    {
        experiment(numberOfUEs, phyMode1, phyMode2, verbose, m_wifiPhyStats, m_mobility, m_protocol, m_numberOfInterfaces, duration);
